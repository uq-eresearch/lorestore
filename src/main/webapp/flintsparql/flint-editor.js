...
function FlintEditor(container, imagesPath, config) {
	...
	var initialQuery = "SELECT * WHERE {?s ?p ?o}\nLIMIT 10"
	...
	try {
	    ...
	    if ($.browser.msie) {
		$('#' + container).append("<form id='" + editorId + "' action='" + config.endpoints[0].uri + "' method='get'></form>");
	    }
	    else {
		...
		try {
			...
			success: function(data) {resultsArea.setResults(data);}
		    });
		}
		catch (e) {errorBox.show(e);}
	    }
	    ...
	    var keywords= /^(BASE|PREFIX|SELECT|FROM|NAMED|ORDER|BY|LIMIT|ASC|DESC|OFFSET|DISTINCT|REDUCED|WHERE|GRAPH|OPTIONAL|UNION|FILTER|STR|LANG|LANGMATCHES|DATATYPE|BOUND|SAMETERM|ISIRI|ISURI|ISBLANK|ISLITERAL|REGEX|TRUE|FALSE)$/i;
