...
//		st.loadData();
	...
	public void trialKeywordSearch(String searchString) throws Exception {
		...
		for (int i = 0; i < numTests; i++) {
			...
			ModelAndView searchQuery = qh.searchQuery(null, null, searchString, "date", 0, -1, false, true);
	...
	public void trialAdvancedSearch(String searchString) throws Exception {
		...
		for (int i = 0; i < numTests; i++) {
			long startTime = System.currentTimeMillis();
			qh.searchQuery(null, "http://purl.org/dc/elements/1.1/creator", searchString, "date", 0, -1, false, true);
			long stopTime = System.currentTimeMillis();
			long elapsedTime = stopTime - startTime;
			System.out.println(elapsedTime);
		}
